...
contract VaultWithdrawFuzzTest is BaseFuzzTest {
    ...
    function testFuzzPartialWithdrawalThroughFactoryBurnsSharesAndReturnsAssets(
        uint256 ownerQuoteSeed,
        uint256 ownerBaseSeed,
        uint256 bobQuoteSeed,
        uint256 bobBaseSeed,
        uint256 sharesSeed
    ) public {
        uint256 ownerQuote = _boundVaultQuote(ownerQuoteSeed);
        uint256 ownerBase = _boundVaultBase(ownerBaseSeed);
        address vault = _deployVault(carol, ownerQuote, ownerBase);
        uint256 bobQuote = bound(bobQuoteSeed, _minFollowOn(ownerQuote, MARKET_MIN_SIZE), ownerQuote);
        uint256 bobBase = bound(bobBaseSeed, _minFollowOn(ownerBase, MIN_ETH_FUZZ_AMOUNT), ownerBase);
        uint256 bobShares = _depositIntoVault(bob, vault, bobQuote, bobBase);
    ...
    function testFuzzFullOwnerWithdrawalViaCloseReturnsAssetsAndMarksVaultClosed(
        uint256 ownerQuoteSeed,
        uint256 ownerBaseSeed
    ) public {
        uint256 ownerQuoteAmount = _boundVaultQuote(ownerQuoteSeed);
        uint256 ownerBaseAmount = _boundVaultBase(ownerBaseSeed);
