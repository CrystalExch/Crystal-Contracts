...
contract FallbackBatchFuzzTest is BaseFuzzTest {
    function testFuzzCrystalFallbackPlacesBuyLimitOrderFromEncodedCalldata(uint256 quotePerBaseSeed, uint256 sizeSeed)
        public
    {
    ...
    function testFuzzCrystalFallbackExecutesMultipleLimitActionsFromOneCalldata(
        uint256 bidQuotePerBaseSeed,
        uint256 askQuotePerBaseSeed,
        uint256 bidSizeSeed,
        uint256 askSizeSeed
    ) public {
        (uint256 bidPrice, uint256 askPrice) = _boundOrderedPrices(bidQuotePerBaseSeed, askQuotePerBaseSeed);
        uint256 bidSize = _boundQuoteAmount(bidSizeSeed);
        uint256 askSize = _boundSellSize(askSizeSeed, askPrice);
    ...
    function testFuzzCrystalFallbackInternalBalanceModeLocksDepositedQuote(uint256 quotePerBaseSeed, uint256 sizeSeed)
        public
    {
        uint256 price = _boundPrice(quotePerBaseSeed);
        uint256 size = _boundInternalQuoteAmount(sizeSeed);
