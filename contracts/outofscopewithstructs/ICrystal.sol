...
interface ICrystal {
    struct InternalOrder { //  bit is if maker wants internal balance (1) or tokens (0) order is stored at either marketid << 128 | price << 48 | id or cloid << 41 | userid; no collision because marketid seperates cloid orders from non cloid, userid prevents cloid collisions, and price n id are always unique
        uint112 size; //uint112 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFF
        uint8 orderType; // uint1 0x1
        uint40 userId; // uint41 0x1FFFFFFFFFF
        uint48 fillBefore; // uint51 0x7FFFFFFFFFFFF
        uint48 fillAfter; // uint51 0x7FFFFFFFFFFFF
    }
    ...
    struct PriceLevel { 
        uint112 size; // uint112 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFF
        // gap uint1 0x1
        uint40 latestNativeId; // uint41 0x1FFFFFFFFFF
        uint48 latest; // uint51 0x7FFFFFFFFFFFF
        uint48 fillNext; // uint51 0x7FFFFFFFFFFFF
    }
