...
import {CrystalMarketUnoptimized} from './CrystalMarket.sol';

contract CrystalUnoptimized is ICrystal {
    ...
    mapping (uint256 => ICrystal.InternalOrder) private orders; // 0 is an invalid cloid, valid range 1-1023 mask 0x3FF; marketid << 128 | price << 48 | id or cloid << 41 | userid
    ...
    constructor(address _weth, address _gov, address _feeRecipient, uint8 _feeCommission, uint8 _feeRebate, uint256 _feeClaimDuration, ICrystal.LaunchpadParams memory _launchpadParams) {
        ...
        require((_feeCommission + _feeRebate) < 50 && minSize < 0xFFFFF && minSizeZeroes < 0xFFFFF && _launchpadParams.launchpadInitialNativeSupply > 1e18 && 90000 <= _launchpadParams.launchpadFee && _launchpadParams.launchpadFee <= 100000 && 90000 <= _launchpadParams.graduatedTakerFee);
    ...
    function _verifyUser(address user) internal returns (address) {
        if (user == address(0)) {
            user = msg.sender;
        }
    ...
    function getAllOrdersByCloid(address user, uint256 range) external view returns (uint256[] memory cloids, ICrystal.Order[] memory userOrders) {
        ...
        for (uint256 i = 1; i < (range > 1024 ? 1024 : range); ++i) {
            uint256 order = orders[(i << 41) | userId].size;
            if (order != 0) {
                temp[count++] = i;
            }
        }
    ...
    function getOrderByCloid(uint256 userId, uint256 cloid) public view returns (ICrystal.Order memory) {
        ICrystal.InternalOrder memory order = orders[(cloid << 41) | userId];
        ...
        return ICrystal.Order(price <= _getMarket[market].highestBid ? true : false, market, price, order.size, order.orderType, order.userId, order.fillBefore, order.fillAfter);
    }
    
    function getOrder(address market, uint256 price, uint256 id) external view returns (ICrystal.Order memory) {
        ICrystal.InternalOrder memory order = orders[(marketToMarketId[market] << 128) | (price << 48) | id];
        return ICrystal.Order(price <= _getMarket[market].highestBid ? true : false, market, price, order.size, order.orderType, order.userId, order.fillBefore, order.fillAfter);
    }

    function getPriceLevel(address market, uint256 price) external view returns (ICrystal.PriceLevel memory) {
        return priceLevels[(marketToMarketId[market] << 128) | price];
    }
    // market methods
    ...
    function deploy(bool isCanonical, address quoteAsset, address baseAsset, uint256 marketType, uint256 scaleFactor, uint256 tickSize, uint256 maxPrice, uint256 minSize, uint24 takerFee, uint24 makerRebate) external returns (address market) {
        ...
        uint256 marketId = allMarkets.length + 1;
        {
            ...
            uint256 maxTick;
            market = address(new CrystalMarketUnoptimized{salt: keccak256(abi.encode(marketId))}());
            if (marketType == 0) {
            ...
            require(tempMinSize < 0xFFFFF && minSizeZeroes < 0xFFFFF && marketId < 0xFFFFFFFFFFFF); // minSize is encoded as bits 20-40 * 10 ** bits 0-20, marketid max uint48 minsize is variable to prevent dos
        ...
        if (isCanonical) {
            ...
            require(launchpadTokenToMarket[baseAsset].virtualNativeReserve == 0);
    ...
    function clearCloidSlots(uint256 userId, uint256[] calldata ids) external {
        ...
        for (uint256 i; i < ids.length; ++i) {
            ...
            if ((orders[(ids[i] << 41) | userId].size) == 0) {
    ...
    function changeMarketParams(address market, uint256 newMinSize, uint24 newTakerFee, uint24 newMakerRebate, bool isAMMEnabled, bool isCanonical) external {
        ...
        emit ICrystal.MarketParamsChanged(market, (m.minSize >> 20) * 10 ** (m.minSize & 0xFFFFF), newTakerFee, newMakerRebate, m.isAMMEnabled);
    }
    ...
    function changeLaunchpadParams(ICrystal.LaunchpadParams memory newLaunchpadParams) external {
        ...
        require(minSize < 0xFFFFF && minSizeZeroes < 0xFFFFF && newLaunchpadParams.launchpadInitialNativeSupply > 1e18 && 90000 <= newLaunchpadParams.launchpadFee && newLaunchpadParams.launchpadFee <= 100000 && 90000 <= newLaunchpadParams.graduatedTakerFee);
    ...
    function createToken(string memory name, string memory symbol, string memory metadataCID, string memory description, string memory social1, string memory social2, string memory social3) external returns (address token) {
        ...
        emit ICrystal.TokenCreated(address(token), msg.sender, name, symbol, metadataCID, description, social1, social2, social3);
        uint256 marketId = allMarkets.length + 1;
        parameters = ICrystal.Parameters(weth, token, marketId, 3, 9, 1, 1000000000000000); // maxsize is validated here
        uint256 maxTick;
        address market;
        market = address(new CrystalMarketUnoptimized{salt: keccak256(abi.encode(marketId))}()); // can't be traded yet as slot 0 in mapping isn't initialized, and is not in getmarketbyassets either
        ...
        (m.maxPrice, m.marketType, m.creator, m.creatorFeeSplit) = (1000000000000000, 3, msg.sender, uint8(launchpadParams.graduatedCreatorFeeSplit));
        activated[(marketId << 128)] = 1; // index 0
        activated[(marketId << 128) | (maxTick >> 8)] = (1 << (maxTick % 256));
        allMarkets.push(market);
        marketToMarketId[market] = marketId;
        marketIdToMarket[marketId] = market;
        launchpadTokenToMarket[address(token)] = ICrystal.LaunchpadMarket(launchpadParams.launchpadInitialNativeSupply, 1000000000000000000000000000, uint256(launchpadParams.launchpadInitialNativeSupply) * 1000000000000000000000000000, msg.sender, market);
        (bool result, bytes memory ret) = market.delegatecall(
    ...
    function buy(bool isExactInput, address token, uint256 amountIn, uint256 amountOut) external payable returns (uint256, uint256, bool) {
        ...
        if (launchpadMarket.virtualTokenReserve != 0) {
            ...
            if (launchpadMarket.virtualNativeReserve >= ((launchpadMarket.k / 200000000000000000000000000))) { // graduate
                ...
                ICrystal.Market storage m = _getMarket[market];
                {
                    ...
                    (m.lowestAsk, m.minSize, m.takerFee, m.makerRebate, m.isAMMEnabled) = (uint80(1000000000000000), uint40((tempMinSize << 20) | minSizeZeroes), uint24(launchpadParams.graduatedTakerFee), uint24(launchpadParams.graduatedMakerRebate), true); // init market
                }
        ...
        return (amountIn, amountOut, launchpadMarket.market == address(0));
    }
